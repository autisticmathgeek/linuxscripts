...
for i in "${!TARGET_DEVICES[@]}"; do
    ...
    # 3.2. Use a simple, robust SED command to ensure 'defaults,nofail,x-systemd.device-timeout=60s' is in the options field (column 4) and '0 2' in the dump and pass fields (columns 5 6)
    # This prevents the system from hanging if the drive is disconnected and ensures that a fsck is performed on the device before mounting
   ...
   sed -i -E "/[[:space:]]+${mp_re}[[:space:]]/ {
    ...
    echo "   -> Successfully ensured desired mount options are present "
...
if mount -a; then
    ...
    echo "**NOTE--> THIS FAILURE IS EXPECTED IF YOU HAVE FEWER THAN 4 HDDS CONNECTED**"
